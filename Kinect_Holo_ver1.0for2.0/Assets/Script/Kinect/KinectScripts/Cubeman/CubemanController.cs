...
public class CubemanController : MonoBehaviour 
{
    ...
    public float sensitivityZ = 1.0f;
...
//       	initialPosition = new Vector3(transform.position.x * sensitivityX, transform.position.y * sensitivityY, transform.position.z * sensitivityZ);
		...
		for(int i = 0; i < bones.Length; i++) 
		{
			if(bones[i] != null)
			{
				...
				if(manager.IsJointTracked(playerID, joint))
				{
                    ...
                    bones[i].transform.localPosition = posJoint;    //ㅎㅇ *30; 큐브간 간격이 멀어짐, 조인트 별 민감도 조정 가능
                    bones[i].transform.rotation = rotJoint;
                    //ㅎㅇ *30; 큐브간 간격도 멀어지고, 걷기에 따른 민감도 상승
                    //지금은 x,y,z별로 괄호 닫을때까지는 작성 할 예정
                    Vector3 temp = bones[i].transform.position;
                    bones[i].transform.position = new Vector3(temp.x*sensitivityX,temp.y * sensitivityY, temp.z*sensitivityZ);
                }
                else
				{
