//This is the main Script that makes the RotationalMenuScript.

using UnityEngine;
using System.Collections;
using System.Collections.Generic; //this allows us to make a list variable

public class RotationalMenuScript : MonoBehaviour 
{
	...
	public AudioClip RotateAudio; //AudioFile to play on rotate
	...
	void Start () 
	{
		...
		{
			RotationIndex = PlayerPrefs.GetInt("Scene" + Application.loadedLevel.ToString() + "FocusOption"); //assign that RotationIndex
		}
	...
	void Update()
	{
		...
		if (Input.GetButtonDown(Axis) && Input.GetAxis(Axis) > 0f) //rotates the RotationalMenu
		{
	...
	{
		GetComponent<AudioSource>().GetComponent<AudioSource>().PlayOneShot(RotateAudio);
	}
}
